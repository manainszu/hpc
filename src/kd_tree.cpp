...
static float
medianPartition( size_t* ofs, int a, int b, const float* vals )  //中位数划分
{                      //索引数组   //左右区间   //原数组
    int k, a0 = a, b0 = b;//
    int middle = (a + b)/2;  //计算中位数的位置
    float32
    while( b > a )
    {
        ...
        //三点中值法 就是简单的比较
        // uint32x4_t vcgtq_s32(float32x4_t v0,float32x4_t v1)
...
static void //表示作用域仅限于当前编译单元
computeSums( const Mat& points, const size_t* ofs, int a, int b, double* sums )
{
    int i, j, dims = points.cols; //获取points的特征维度
    const float* data = points.ptr<float>(0);  //获取第0行首地址  //*data解引用指针
    for( j = 0; j < dims; j++ )
